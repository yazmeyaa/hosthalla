...
const hostSelectColumns = "id, name, description, ip, created_at, updated_at"
...
func (h HostRepositoryPostgresImpl) CreateHost(ctx context.Context, data storage.CreateHostDTO) (host.Host, error) {
	const insertHostQuery = "insert into host (name, description, ip) values ($1, $2, $3) returning id, name, description, ip, created_at, updated_at"
	row := h.pool.QueryRow(ctx, insertHostQuery, data.Name, data.Description, data.IP)
	return scanHost(row)
}
...
func (h HostRepositoryPostgresImpl) UpdateHost(ctx context.Context, host *host.Host) error {
	...
	row := h.pool.QueryRow(ctx, updateHostQuery, uuid.UUID(host.ID), host.Name, host.Description, host.IP)
